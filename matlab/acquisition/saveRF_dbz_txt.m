...
disp('saveRF_dbz_txt: start');
runningVSX = ~isempty(findobj('tag','UI'));
fprintf('saveRF_dbz_txt: runningVSX = %d\n', runningVSX);
if runningVSX % running VSX
    freezeVal = evalin('base','freeze');
    fprintf('saveRF_dbz_txt: freeze = %d\n', freezeVal);
    if freezeVal==0   % no action if not in freeze
        disp('saveRF_dbz_txt: VSX not frozen — aborting. Press Freeze first.');
        msgbox('Please freeze VSX');
        return
    else
        disp('saveRF_dbz_txt: calling copyBuffers...');
    ...
    disp('saveRF_dbz_txt: not running VSX, checking for RcvData in base...');
    if evalin('base','exist(''RcvData'',''var'');')
        RcvData = evalin('base','RcvData');
    else
        disp('saveRF_dbz_txt: RcvData does not exist! Aborting.');
        return
    end
end
fprintf('saveRF_dbz_txt: RcvData{2} class=%s size=%s\n', ...
...
fprintf('saveRF_dbz_txt: target folder = %s (exists=%d)\n', filepath, isfolder(filepath));
...
fprintf('saveRF_dbz_txt: writing RF data to %s\n', save_RFfilename);
...
fprintf('saveRF_dbz_txt: writing workspace file to %s\n', save_workspace_filename);
evalin('base', sprintf("save('%s')", strrep(save_workspace_filename,'\','/')));

fprintf('saveRF_dbz_txt: done. Saved RF data, size file, and workspace to:\n  %s\n', filepath);
fprintf('saveRF_dbz_txt: base name: %s\n', RFfilename);
